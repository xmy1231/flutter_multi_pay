...
import com.plugin.multipay.wxapi.WechatResultHolder
...
class MultiPayPlugin : FlutterPlugin, MethodChannel.MethodCallHandler, ActivityAware,
    ...
    private var isUnionPayPending = false
    ...
    private fun payWithUnionPay(orderInfo: String) {
        ...
        try {
            ...
            val ret = UPPayAssistEx.startPay(activity, orderInfo, mode, null, null)
            if (ret == 0) {
                isUnionPayPending = true
            } else {
                pendingResult?.error("UNIONPAY_ERROR", "Failed to start UnionPay: $ret", null)
                pendingResult = null
            }
        } catch (e: Exception) {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        if (isUnionPayPending && data != null) {
            val payResult = data.getStringExtra("pay_result")
            when (payResult) {
                "success" -> pendingResult?.success(mapOf("channel" to "unionPay", "status" to "success"))
                "fail" -> pendingResult?.success(
                    mapOf("channel" to "unionPay", "status" to "failed", "errorCode" to data.getStringExtra("error_code"))
                )
                "cancel" -> pendingResult?.success(mapOf("channel" to "unionPay", "status" to "cancelled"))
            }
            if (payResult != null) {
                pendingResult = null
                isUnionPayPending = false
            }
            return true
        }
        return false
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        (binding.lifecycle as Lifecycle).addObserver(LifecycleEventObserver { _, event ->
